...
const protectAdminActions = catchAsync(async (req, res, next) => {
    const { sessionUser } = req;

    if(sessionUser.role !== "admin"){
        return next(new AppError('You are not authorized to make this action'), 403);
    }

    next();
});

module.exports = { protectSession, protectUserAccount, protectAdminActions };